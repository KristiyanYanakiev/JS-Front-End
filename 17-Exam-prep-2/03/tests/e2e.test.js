...
const host = 'http://127.0.0.1:5500/17-Exam-prep-2/03/index.html'; // Application host (NOT service host - that can be anything)
...
const endpoints = {
  catalog: '/jsonstore/orders',
  byId: (id) => `/jsonstore/orders/${id}`,
};
...
//   );
  ...
  describe('Order Catalog Schedule Tests', () => {
    it('Load All Orders', async () => {
      const data = mockData.list;
      const { get } = await handle(endpoints.catalog);
      get(data);

      await page.goto(host);
      await page.waitForSelector('#load-orders');

      await page.click('#load-orders');

      const list = await page.$$eval(`#order-info #list .container`, (t) =>
   ...
   it('Create Order', async () => {
      ...
      await page.fill('#quantity', data.quantity);
      ...
      expect(postData.quantity).to.equal(data.quantity);
      expect(postData.date).to.equal(data.date);

      const [name] = await page.$$eval(`#name`, (t) =>
        t.map((s) => s.value)
      );
      const [numquantity] = await page.$$eval(`#quantity`, (t) =>
        t.map((s) => s.value)
      );
      ...
      expect(numquantity).to.equal('');
      expect(date).to.equal('');
    });

    it('Edit Order (Has Input)', async () => {
      await page.goto(host);
      const data = mockData.list[0];
      ...
      expect(allCourse[1]).to.include(data.quantity);
      expect(allCourse[2]).to.include(data.date);

    });

    it('Edit Order (Makes API Call)', async () => {
      const data = mockData.list[0];
      await page.goto(host);
